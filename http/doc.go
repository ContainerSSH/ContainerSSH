// Package http provides a much simplified API for both the HTTP client and server. It is meant for use in conjunction
// with ContainerSSH and should not be used for other purposes as the implementation is very ContainerSSH-specific.
//
// Using the client
//
// The client library takes a request object that can be marshalled into JSON format (see https://gobyexample.com/json)
// and sends it to the server. It then fills a response object with the response received from the server. In code:
//
...
//     }
//
//     // Now the response variable will contain the decoded response.
//
...
//
// Like before, the logger parameter is a logger from the github.com/containerssh/libcontainerssh/log package. The handler
// is a regular go HTTP handler (see https://golang.org/pkg/net/http/#Handler) that satisfies this interface:
//
...
// (https://github.com/containerssh/libcontainerssh/service).
//
// Using a simplified handler
//
// This package also provides a simplified handler that helps with encoding and decoding JSON messages. It can be
// created as follows:
//
...
//     }
//
// In other words, the ServerRequest object gives you the ability to decode the request into a struct of your choice.
// The ServerResponse, conversely, encodes a struct into the the response body and provides the ability to enter a
// status code.
//
// Content negotiation
//
// If you wish to perform content negotiation on the server side, this library now supports switching between text and
// JSON output. This can be invoked using the NewServerHandlerNegotiate method instead of NewServerHandler. This handler
// will attempt to switch based on the `Accept` header sent by the client. You can marshal objects to text by
...
//
// This is a very simple handler example. You can use utility like gorilla/mux (https://github.com/gorilla/mux) as an
// intermediate handler between the simplified handler and the server itself.
package http
